...
final class SubmissionController {
	...
	public static function submit( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		if ( ! self::is_same_origin( $request ) || ! wp_verify_nonce( (string) $request->get_header( 'X-Gama-Contact-Nonce' ), 'gama_contact_submit' ) ) {
			return new WP_Error( 'gama_contact_forbidden', __( 'Nie udało się zweryfikować formularza. Odśwież stronę i spróbuj ponownie.', 'gama-contact' ), array( 'status' => 403 ) );
		}

		$input = (array) $request->get_json_params();
		if ( sanitize_text_field( (string) ( $input['company'] ?? '' ) ) !== '' ) {
			return new WP_REST_Response( array( 'message' => __( 'Dziękujemy. Wiadomość została przyjęta.', 'gama-contact' ) ), 200 );
		}
		if ( ! RateLimiter::consume() ) {
			return new WP_Error( 'gama_contact_rate_limited', __( 'Wysłano zbyt wiele wiadomości. Spróbuj ponownie później.', 'gama-contact' ), array( 'status' => 429 ) );
		}
		...
		if ( ! is_email( $recipient ) ) {
			return new WP_Error( 'gama_contact_configuration', __( 'Formularz jest chwilowo niedostępny.', 'gama-contact' ), array( 'status' => 503 ) );
		}

		$headers = array( 'Content-Type: text/plain; charset=UTF-8', 'Reply-To: ' . $values['name'] . ' <' . $values['email'] . '>' );
		if ( is_email( $sender ) ) {
		...
		if ( ! wp_mail( $recipient, __( 'Nowa wiadomość ze strony Gama Software', 'gama-contact' ), $body, $headers ) ) {
			return new WP_Error( 'gama_contact_delivery', __( 'Nie udało się wysłać wiadomości. Spróbuj ponownie później.', 'gama-contact' ), array( 'status' => 503 ) );
		}

		return new WP_REST_Response( array( 'message' => __( 'Dziękujemy. Wiadomość została wysłana.', 'gama-contact' ) ), 200 );
	}
