...
runtime_assertion="$ROOT_DIR/tests/assert-theme-navigation-runtime.php"
...
if ! grep -Fq 'paused_at_incomplete_token' "$file"; then
  echo 'Navigation bridge must fail closed when Core pauses at an incomplete token.' >&2
  exit 1
fi
grep -Fq 'complete target fixture unexpectedly paused at an incomplete token' "$runtime_assertion"
grep -Fq 'incomplete target markup changed' "$runtime_assertion"
