...
ZIP_PATH="$DIST_DIR/gama-contact-0.3.2.zip"
MANIFEST_PATH="$DIST_DIR/gama-contact-0.3.2.manifest.txt"
SHA_PATH="$DIST_DIR/gama-contact-0.3.2.zip.sha256"
...
gama-contact/src/Form/RateLimiter.php
...
if [[ "$entry_count" -ne 25 ]]; then
  echo "Expected 25 normalized archive entries; found $entry_count." >&2
  exit 1
fi
...
unzip -p "$ZIP_PATH" gama-contact/gama-contact.php | grep -F 'Version: 0.3.2'
...
unzip -p "$ZIP_PATH" gama-contact/readme.txt | grep -F 'Stable tag: 0.3.2'
